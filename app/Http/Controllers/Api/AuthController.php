    ...
    public function register(Request $request)
    {
        ...
        try {
            ...
            if ($request->type === 'office') {
                $expiryDate = now()->addYear(); // إضافة سنة واحدة من تاريخ اليوم
            }
    
            // تحديد حالة الموافقة من قبل المسؤول
            $adminApproved = $request->type === 'user'; // إذا كان نوع المستخدم "user" يتم تفعيل الحقل، وإلا يكون غير مفعل
            ...
            // إضافة بيانات المكتب إذا كانت متوفرة
            if (!empty($request->identity_number)) {
                $imagePath = $request->file('image')->getClientOriginalName();
    
                // حفظ الصورة في المجلد public/uploads/property
                $request->file('image')->storeAs('uploads/commercial_register_image', $imagePath, 'public');
                OfficeDetail::create([
                    'user_id' => $user->id,
